...
void Parser::synchronize() {
    static const unordered_set<string> statementStarters = {
        "yoho", "firta", "voyo", "aghibad", "bhan", "sun",
        "yedi", "yochai", "jabsamma", "gar", "ferini",
...
NodePtr Parser::parseProgram() {
    auto prog = make_unique<Node>(NodeKind::Program, "program", 1, 1);
    while (!isAtEnd()) {
        try {
...
NodePtr Parser::statement() {
    ...
    if (checkKeyword("voyo"))    return breakStatement();
    if (checkKeyword("aghibad")) return continueStatement();
    ...
    if (checkKeyword("jabsamma"))return whileStatement();
...
NodePtr Parser::breakStatement() {
    const Token& kw = expectKeyword("voyo", "expected 'voyo'");
    expect(SEMICOLAN, "expected ';' after 'voyo'");
    return make_unique<Node>(NodeKind::Break, "voyo", kw.line, kw.column);
}

NodePtr Parser::continueStatement() {
    const Token& kw = expectKeyword("aghibad", "expected 'aghibad'");
    expect(SEMICOLAN, "expected ';' after 'aghibad'");
    return make_unique<Node>(NodeKind::Continue, "aghibad", kw.line, kw.column);
}
...
NodePtr Parser::ifStatement() {
    ...
    while (checkKeyword("tesovaya")) {
        advance();
        expect(LEFT_PAREN, "expected '(' after 'tesovaya'");
        auto elifCond = expression();
        expect(RIGHT_PAREN, "expected ')' after 'tesovaya' condition");
...
NodePtr Parser::whileStatement() {
    const Token& kw = expectKeyword("jabsamma", "expected 'jabsamma'");
    expect(LEFT_PAREN, "expected '(' after 'jabsamma'");
    auto cond = expression();
    expect(RIGHT_PAREN, "expected ')' after while condition");

    if (!check(LEFT_BRECE)) {
        error(peek(), "expected '{' to start 'jabsamma' body");
    }
    auto body = block();

    auto node = make_unique<Node>(NodeKind::While, "jabsamma", kw.line, kw.column);
    node->children.push_back(move(cond));
...
NodePtr Parser::doWhileStatement() {
    ...
    auto body = block();
    expectKeyword("jabsamma", "expected 'jabsamma' after do-block");
    expect(LEFT_PAREN, "expected '(' after 'jabsamma'");
    auto cond = expression();
